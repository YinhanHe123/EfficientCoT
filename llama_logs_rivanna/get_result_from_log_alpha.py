...
def process_alpha_directories(base_path, alpha_values):
    results = []

    for alpha in alpha_values:
        alpha_dir = f"{base_path}/alpha_0.{alpha}/output.log"
        print(f"Processing {alpha_dir}...")

        if not os.path.exists(alpha_dir):
            print(f"Warning: Log file not found at {alpha_dir}")
            continue

        # Extract values
        numerical_accuracy_values, ave_sample_time_values = extract_values_from_log(alpha_dir)

        if numerical_accuracy_values is None or ave_sample_time_values is None:
            print(f"Failed to extract values from {alpha_dir}")
            continue
...
def main():
    ...
    output_csv = "/home/nee7ne/EfficientCoT/logs/alpha_analysis_results.csv"

    # Process all directories
    results = process_alpha_directories(base_path, alpha_values)
    ...
    if success:
        ...
        print("\nSummary of results:")
        df = pd.DataFrame(results)
        print(df.groupby('alpha')[['accuracy_mean', 'sample_time_mean']].mean())
