...
class CCoTModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, output_hidden_states=False, target_states=None, max_contemplation_tokens=None): #target_states is None is set to avoid 'target_states' signiture removal when data is passed to trainer()
        ...
        Args:
            ...
            target_states: Optional target hidden states for training
            max_contemplation_tokens: Maximum number of contemplation tokens to generate
...
class CCOTDecodeModel(nn.Module):
    ...
    def apply_lora(self, rank=64, alpha=32):
        """
