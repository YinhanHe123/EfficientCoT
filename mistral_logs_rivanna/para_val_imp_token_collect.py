...
# Base directory
base_dir = "/home/nee7ne/EfficientCoT/llama_logs_rivanna"

# Updated pattern to catch val_imp_token directories
dataset_pattern = re.compile(r"([a-zA-Z0-9_]+)_val_imp_token_(\d+)")

# Find all val_imp_token directories
token_dirs = glob.glob(os.path.join(base_dir, "*_val_imp_token_*"))

# Add debug prints
print(f"Found {len(token_dirs)} val_imp_token directories")
    ...
    if match:
        ...
        for file in csv_files:
            try:
                ...
                # Add columns to identify dataset and token value
                df["dataset"] = dataset
                df["token"] = token_value
...
for dataset, dfs in all_dfs.items():
    if dfs:
        ...
        # Sort by token value (convert to int for proper numerical sorting)
        dataset_df["token"] = dataset_df["token"].astype(int)
        dataset_df = dataset_df.sort_values("token")
...
if combined_dfs:
    ...
    # Reorder columns to have dataset and token as first columns
    cols = result_df.columns.tolist()
    cols = ["dataset", "token"] + [col for col in cols if col not in ["dataset", "token"]]
    result_df = result_df[cols]

    # Save the combined dataframe
    output_path = os.path.join(base_dir, "combined_val_imp_token_results.csv")
