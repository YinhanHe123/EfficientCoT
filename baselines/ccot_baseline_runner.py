...
def run_ccot_baseline(train_dataset, eval_dataset, model_config, experiment_config):
    ...
    if experiment_config.ccot_stage == "encode":
        if not os.path.exists(ccot_model_path+'/config.pt'):
            ...
            ccot_model = train_ccot_model(
                ...
                learning_rate=experiment_config.ccot_lr,
    ...
    if experiment_config.ccot_stage == "decode":
        if not os.path.exists(decode_model_path+'/config.pt'):
            ...
            decode_model = train_ccot_decode_model(
                ...
                learning_rate=experiment_config.ccot_lr / 10,  # Lower learning rate for decoder
                num_epochs=3,
                batch_size=2,
                device=device
            )
        ...
        for temp in [0.1, 0.3, 0.5, 0.7, 0.9]:
            ...
            with torch.no_grad():
                ...
                for sample in tqdm(eval_dataset, desc="Getting contemplations"):
                    ...
                    contemplation_states_list.append(contemplation_states.half().cpu().numpy())
                ...
                for sample in tqdm(eval_dataset, desc="decoding"):
                    ...
                    contemplation_states = torch.tensor(contemplation_states_list.pop(0), dtype=torch.bfloat16).to(device)
                    ...
                    def modified_prepare_inputs(input_ids, past_key_values=None, **kwargs):
                        ...
                        if len(past_key_values.key_cache) == 0:
                            ...
                            contemp_len = min(contemplation_states.shape[1], experiment_config.eval_max_contemp_tokens)
                    ...
                    outputs = decode_model.model.generate(
                        inputs.input_ids,
                        max_length=30 + inputs.input_ids.size(1) + experiment_config.eval_max_contemp_tokens,  # Account for the input length
                    ...
                    results.append({
                        ...
                        "prediction": answer,
                        "sample_time": decode_time[-1] +contemp_gen_time[-1]
                    })
                del decode_model
                gc.collect()
                torch.cuda.empty_cache()
    ...
    if experiment_config.ccot_stage == "cotrain_encode_decode":
        ...
        ccot_model, decode_model = cotrain_encode_decode(
            ...
            autoregressive_layer=experiment_config.autoregressive_layer,
            learning_rate=experiment_config.ccot_lr / 20,  # Lower learning rate for cotuning
            num_epochs=1,
            batch_size=2,
            device=device
        )
